...
CONF_BURN_RATE_IDLE = "burn_rate_idle"
CONF_BURN_RATE_FULL = "burn_rate_full"
...
schema = {
    ...
    cv.Optional(CONF_BURN_RATE_IDLE, default=0): cv.float_,
    cv.Optional(CONF_BURN_RATE_FULL, default=0): cv.float_,
...
async def to_code(config):
    ...
    if config[CONF_BURN_RATE_IDLE] > 0:
        cg.add(var.set_burn_rate_idle(config[CONF_BURN_RATE_IDLE]))
    if config[CONF_BURN_RATE_FULL] > 0:
        cg.add(var.set_burn_rate_full(config[CONF_BURN_RATE_FULL]))
